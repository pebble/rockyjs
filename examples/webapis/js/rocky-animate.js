...
(function() {
  Rocky.WebAPIBinding.prototype.tween = function(animation) {
    ...
    var handler = function(ctx) {
      var t = Math.min(1, (new Date().getTime() - introStart) / duration);
      t = easing(t);
      step(t, ctx);
      if (t < 1) {
        this.requestRender();
      } else {
        this.off('beforerender', handler);
      }
    }.bind(this);

    this.on('beforerender', handler);
  };
