            ...
            rocky.update_proc = function(ctx, bounds) {
                enhanced_context = {
                    ...
                    fillRect: function(rect) {
                        var rocky = Rocky.activeBinding;
                        rocky.graphics_fill_rect(ctx, [rect.origin.x, rect.origin.y, rect.size.w, rect.size.h]);
                    },
                    fillCircle: function(center, radius) {
                        var rocky = Rocky.activeBinding;
                        var rect = rocky.GRect(center.x - radius, center.y - radius, radius * 2, radius * 2);
                        rocky.graphics_fill_radial(ctx, rect, rocky.GOvalScaleModeFillCircle, radius * 2, 0, Math.PI * 2);
                    }
                };
